...
local SPELLS = {
	DEADEN = 41410,
	BLISTERING_COLD = 71047,
	TAILSWEEP = 55696,
	FROSTBOLT_VOLLEY = 72906
}
...
mod:RegisterEventsInCombat(
	EventString("SPELL_CAST_START", SPELLS.DEADEN, SPELLS.BLISTERING_COLD),
	EventString("SPELL_CAST_SUCCESS", SPELLS.TAILSWEEP, SPELLS.FROSTBOLT_VOLLEY),
	EventString("SPELL_AURA_APPLIED", SPELLS.DEADEN)
)

--Enrage timer
local enrage_timer = mod:NewBerserkTimer(TIMERS[difficulty].BERSERK)
--Deaden warning and timer
local deaden_warning = mod:NewSpecialWarningYou(SPELLS.DEADEN, nil, nil, nil, 1, 2)
local timer_deaden = mod:NewCDTimer(TIMERS[difficulty].DEADEN_CD, SPELLS.DEADEN, nil, nil, nil, 2)
--Blistering Cold warning and timer
local blistering_cold_warning = mod:NewSpecialWarningGTFO(SPELLS.BLISTERING_COLD, nil, nil, nil, 1, 8)
local timer_blistering_cold = mod:NewCDTimer(TIMERS[difficulty].BLISTERING_COLD_CD, SPELLS.BLISTERING_COLD, nil, nil, nil, 2)
--Frostbolt Volley CD timer
local timer_frostbolt_volley = mod:NewCDTimer(TIMERS[difficulty].FROSTBOLT_VOLLEY_CD, SPELLS.FROSTBOLT_VOLLEY, nil, nil, nil, 2)
--Tailsweep CD timer
local timer_tailsweep = mod:NewCDTimer(TIMERS[difficulty].TAILSWEEP_CD, SPELLS.TAILSWEEP, nil, nil, nil, 2)
...
function mod:SPELL_CAST_START(args)
	--Deaden casting
	if args.spellId == SPELLS.DEADEN then
		timer_deaden:Start(TIMERS[difficulty].DEADEN_CD)
	--Blistering cold cast started
    elseif args.spellId == SPELLS.BLISTERING_COLD then
...
function mod:SPELL_CAST_SUCCESS(args)
	-- Tailsweep
	if args.spellId == SPELLS.TAILSWEEP then
		timer_tailsweep:Start(TIMERS[difficulty].TAILSWEEP_CD)
	-- Frostbolt volley
    elseif args.spellId == SPELLS.FROSTBOLT_VOLLEY then
